...
3.  Make a barplot comparing the elevation of your 3 points in the Poudre Canyon (note: explore the `extract()` function in the `terra` package).
    ...
    ```{r}
    poudre_points_prj$elevation <- extract(elevation, poudre_points_prj)$Elevation #note that 'extract' returns a 2 column df, index to keep just the second column w/ elevation values
...
4.  Why are there 4 features in our Poudre Canyon Highway variable instead of 1?
    ...
    There are some small separate linear features off the Highway path that are grouped with HWY 14.
